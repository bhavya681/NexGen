  ...
  return (
    <header className="bg-blue-600 dark:bg-gray-800 text-white shadow-lg">
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        <h1 className="text-3xl font-extrabold transition-all hover:animate-pulse hover:text-purple-500">
          <Link to="/">NexGen News</Link>
        </h1>
      ...
      {isOpen && (
        <div className="md:hidden bg-blue-600 dark:bg-gray-800 text-white px-4 pt-4 pb-2 shadow-md">
          <ul className="space-y-2">
            {['Latest Updates', 'Apple News', 'Tesla News', 'Cricket News', 'MMA News', 'Olympics News', 'Tech News', 'Startup News', 'Stocks News'].map(
              (item, index) => (
                <li key={index}>
                  <Link
                    to={`/${item.toLowerCase().replace(/ /g, '-')}`}
